...
def draw_face_landmarks(img, landmarks_pre, face_w, face_h, x0, y0, vis=False):
    # img:opencv格式，原始图像
    # landmarks_pre:98关键点，共196个坐标,这里传入的landmarks已经去除掉了bs维度
    # face_w是人脸图像原始图像的宽度(未经过resize)
    # face_h是人脸图像原始图像的高度(未经过resize)
    # x0,y0是图像box左上角
    img_align = img.copy()  # 原始人脸(未经过crop)
    ...
    x_list = []
    y_list = []
    for i in range(int(landmarks_pre.shape[0]/2)):  # 遍历索引坐标 98关键点
        x = landmarks_pre[i*2+0] * float(face_w) + x0  # *2是用来取每个关键点的x坐标
        y = landmarks_pre[i * 2 + 1] * float(face_h) + y0
