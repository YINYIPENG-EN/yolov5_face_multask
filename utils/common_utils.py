...
def draw_landmarks(img, output, or_img, box, draw_circle):
    '''
    img是resize后的图像(256x256)
    '''
    x1, y1, x2, y2 = box
    img_width = img.shape[1]  # resize后的图像w
    img_height = img.shape[0]  # resize后的图像h
    dict_landmarks = {}
    for i in range(int(output.shape[0]/2)):  # 获取98个关键点
        x = output[i*2+0]*float(img_width)
        y = output[i*2+1]*float(img_height)
        if 41>= i >=33:
            if 'left_eyebrow' not in dict_landmarks.keys():
                dict_landmarks['left_eyebrow'] = []
            dict_landmarks['left_eyebrow'].append([int(x+x1),int(y+y1),(0,255,0)])
            if draw_circle:
                cv2.circle(or_img, (int(x+x1),int(y+y1)), 2, (0,255,0), 1)
        elif 50>= i >=42:
            if 'right_eyebrow' not in dict_landmarks.keys():
                dict_landmarks['right_eyebrow'] = []
            dict_landmarks['right_eyebrow'].append([int(x+x1),int(y+y1),(0,255,0)])
            if draw_circle:
                cv2.circle(or_img, (int(x+x1),int(y+y1)), 2, (0,255,0),1)
        elif 67>= i >=60:
            if 'left_eye' not in dict_landmarks.keys():
                dict_landmarks['left_eye'] = []
            dict_landmarks['left_eye'].append([int(x+x1),int(y+y1),(255,0,255)])
            if draw_circle:
                cv2.circle(or_img, (int(x+x1),int(y+y1)), 2, (255,0,255),1)
        elif 75>= i >=68:
            if 'right_eye' not in dict_landmarks.keys():
                dict_landmarks['right_eye'] = []
            dict_landmarks['right_eye'].append([int(x+x1),int(y+y1),(255,0,255)])
            if draw_circle:
                cv2.circle(or_img, (int(x+x1),int(y+y1)), 2, (255,0,255),1)
        elif 97>= i >=96:
            cv2.circle(or_img, (int(x+x1),int(y+y1)), 2, (0,0,255),1)
        elif 54>= i >=51:
            if 'bridge_nose' not in dict_landmarks.keys():
                dict_landmarks['bridge_nose'] = []
            dict_landmarks['bridge_nose'].append([int(x+x1),int(y+y1),(0,170,255)])
            if draw_circle:
                cv2.circle(or_img, (int(x+x1),int(y+y1)), 2, (0,170,255),1)
        elif 32>= i >=0:
            if 'basin' not in dict_landmarks.keys():
                dict_landmarks['basin'] = []
            dict_landmarks['basin'].append([int(x+x1),int(y+y1),(255,30,30)])
            if draw_circle:
                cv2.circle(or_img, (int(x+x1),int(y+y1)), 2, (255,30,30),1)
        elif 59>= i >=55:
            if 'wing_nose' not in dict_landmarks.keys():
                dict_landmarks['wing_nose'] = []
            dict_landmarks['wing_nose'].append([int(x+x1),int(y+y1),(0,255,255)])
            if draw_circle:
                cv2.circle(or_img, (int(x+x1),int(y+y1)), 2, (0,255,255),1)
        elif 87>= i >=76:
            if 'out_lip' not in dict_landmarks.keys():
                dict_landmarks['out_lip'] = []
            dict_landmarks['out_lip'].append([int(x+x1),int(y+y1),(255,255,0)])
            if draw_circle:
                cv2.circle(or_img, (int(x+x1),int(y+y1)), 2, (255,255,0),1)
        elif 95>= i >=88:
            if 'in_lip' not in dict_landmarks.keys():
                dict_landmarks['in_lip'] = []
            dict_landmarks['in_lip'].append([int(x+x1),int(y+y1),(50,220,255)])
            if draw_circle:
                cv2.circle(or_img, (int(x+x1),int(y+y1)), 2, (50,220,255),1)
        else:
            if draw_circle:
                cv2.circle(or_img, (int(x+x1),int(y+y1)), 2, (255,0,255),1)
